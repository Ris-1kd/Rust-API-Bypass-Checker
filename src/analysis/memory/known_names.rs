...
impl KnownNamesCache {
    ...
    fn get_known_name_for(tcx: TyCtxt<'_>, def_id: DefId) -> KnownNames {
        ...
        let get_known_name_for_num_int_namespace = |mut def_path_data_iter: Iter<'_>| {
            // core::num::<int>::checked_add
            // path looks like: ["core", "num", "i128", "checked_add"]
            def_path_data_iter.next(); // skip the concrete int type like i128/u64
            get_path_data_elem_name(def_path_data_iter.next())
                .map(|n| match n.as_str() {
                    "checked_add" => KnownNames::StdIntCheckedAdd,
                    _ => KnownNames::None,
                })
                .unwrap_or(KnownNames::None)
        };
        ...
        let get_known_name_for_known_crate = |mut def_path_data_iter: Iter<'_>| {
            get_path_data_elem_name(def_path_data_iter.next())
                .map(|n| match n.as_str() {
                    ...
                    // integer impl blocks live under core::num::<int>
                    "num" => get_known_name_for_num_int_namespace(def_path_data_iter),
