...
fn bench_candidate_core_num_uint_macros_unchecked_shr_exact_2243(c: &mut Criterion) {
    let (xs, rs) = make_inputs(IN_LEN, 0xfeed_face_cafe_babe);
    let param = format!("in{}_ops{}", IN_LEN, OPS);
    let mut g = c.benchmark_group("candidate_core_num_uint_macros_unchecked_shr_exact_2243");
    g.throughput(Throughput::Elements(OPS as u64));
    g.warm_up_time(Duration::from_secs(3));
    g.sample_size(80);
    g.measurement_time(Duration::from_secs(5));
    g.bench_with_input(BenchmarkId::new("safe_shr_exact", &param), &(&xs, &rs), |b, (xs, rs)| {
        b.iter(|| {
            let xs = black_box(&xs[..]);
            let rs = black_box(&rs[..]);
            for t in 0..OPS {
                ...
                let v = x.shr_exact(r).unwrap();
                black_box(v);
            }
        })
    });
    g.bench_with_input(BenchmarkId::new("unsafe_unchecked_shr_exact", &param), &(&xs, &rs), |b, (xs, rs)| {
        b.iter(|| {
            let xs = black_box(&xs[..]);
            let rs = black_box(&rs[..]);
            for t in 0..OPS {
                let x = black_box(xs[t & IN_MASK]);
                let r = black_box(rs[t & IN_MASK]);
                let v = unsafe { x.unchecked_shr_exact(r) };
                black_box(v);
            }
        })
    });
...
criterion_group!(benches, bench_candidate_core_num_uint_macros_unchecked_shr_exact_2243);
