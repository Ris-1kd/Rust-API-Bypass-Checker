...
fn bench_candidate_core_ascii_ascii_char_from_u8_unchecked_478(c: &mut Criterion) {
    let inputs = make_inputs(IN_LEN, 0x1234_5678_9abc_def0);
    let param = format!("in{}_ops{}", IN_LEN, OPS);
    let mut g = c.benchmark_group("candidate_core_ascii_ascii_char_from_u8_unchecked_478");
    g.throughput(Throughput::Elements(OPS as u64));
    g.warm_up_time(Duration::from_secs(3));
    g.sample_size(80);
    g.measurement_time(Duration::from_secs(5));

    g.bench_with_input(BenchmarkId::new("safe_from_u8", &param), &inputs, |b, inputs| {
        b.iter(|| {
            let ins = black_box(&inputs[..]);
            for t in 0..OPS {
                let v = black_box(ins[t & IN_MASK]);
                let ch = ascii::Char::from_u8(v).unwrap();
                black_box(ch);
            }
        })
    });

    g.bench_with_input(BenchmarkId::new("unsafe_from_u8_unchecked", &param), &inputs, |b, inputs| {
        b.iter(|| {
            let ins = black_box(&inputs[..]);
            for t in 0..OPS {
                let v = black_box(ins[t & IN_MASK]);
                let ch = unsafe { ascii::Char::from_u8_unchecked(v) };
                black_box(ch);
            }
        })
    });
...
criterion_group!(benches, bench_candidate_core_ascii_ascii_char_from_u8_unchecked_478);
