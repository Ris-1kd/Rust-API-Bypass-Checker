...
fn make_inputs(n: usize, seed: u64) -> Vec<usize> {
    let mut x = seed;
    let mut out = Vec::with_capacity(n);
    for _ in 0..n {
        x = x.wrapping_mul(6364136223846793005).wrapping_add(1);
        out.push(1usize << (((x >> 32) as usize % 8) + 1));
    }
    out
}

fn bench_candidate_core_mem_alignment_new_unchecked_155(c: &mut Criterion) {
    let inputs = make_inputs(IN_LEN, 0xfeed_face_cafe_babe);
    let param = format!("in{}_ops{}", IN_LEN, OPS);
    let mut g = c.benchmark_group("candidate_core_mem_alignment_new_unchecked_155");
    ...
    g.bench_with_input(BenchmarkId::new("safe_new", &param), &inputs, |b, inputs| {
        b.iter(|| {
            ...
            for t in 0..OPS {
                ...
                let a = Alignment::new(v).unwrap();
                black_box(a.as_usize());
            }
        })
    });
    ...
    g.bench_with_input(BenchmarkId::new("unsafe_new_unchecked", &param), &inputs, |b, inputs| {
        b.iter(|| {
            ...
            for t in 0..OPS {
                let v = black_box(ins[t & IN_MASK]);
                let a = unsafe { Alignment::new_unchecked(v) };
                black_box(a.as_usize());
            }
        })
    });
...
criterion_group!(benches, bench_candidate_core_mem_alignment_new_unchecked_155);
