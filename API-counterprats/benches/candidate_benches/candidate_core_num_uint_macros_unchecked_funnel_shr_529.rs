...
fn bench_candidate_core_num_uint_macros_unchecked_funnel_shr_529(c: &mut Criterion) {
    let (a, b, rs) = make_inputs(IN_LEN, 0x1234_5678_9abc_def0);
    let param = format!("in{}_ops{}", IN_LEN, OPS);
    let mut g = c.benchmark_group("candidate_core_num_uint_macros_unchecked_funnel_shr_529");
    g.throughput(Throughput::Elements(OPS as u64));
    g.warm_up_time(Duration::from_secs(3));
    g.sample_size(80);
    g.measurement_time(Duration::from_secs(5));
    g.bench_with_input(BenchmarkId::new("safe_funnel_shr", &param), &(&a, &b, &rs), |bch, (a, b, rs)| {
        bch.iter(|| {
            let a = black_box(&a[..]);
            let b = black_box(&b[..]);
            let rs = black_box(&rs[..]);
            for t in 0..OPS {
                ...
                let v = x.funnel_shr(y, r);
                black_box(v);
            }
        })
    });
    g.bench_with_input(BenchmarkId::new("unsafe_unchecked_funnel_shr", &param), &(&a, &b, &rs), |bch, (a, b, rs)| {
        bch.iter(|| {
            let a = black_box(&a[..]);
            let b = black_box(&b[..]);
            let rs = black_box(&rs[..]);
            for t in 0..OPS {
                ...
                let y = black_box(b[t & IN_MASK]);
                let r = black_box(rs[t & IN_MASK]);
                let v = unsafe { x.unchecked_funnel_shr(y, r) };
                black_box(v);
            }
        })
    });
...
criterion_group!(benches, bench_candidate_core_num_uint_macros_unchecked_funnel_shr_529);
