...
fn bench_candidate_core_pin_get_unchecked_mut_1617(c: &mut Criterion) {
    ...
    let mut g = c.benchmark_group("candidate_core_pin_get_unchecked_mut_1617");
    ...
    g.bench_function(BenchmarkId::new("safe", &param), |b| {
        b.iter(|| {
            ...
            for t in 0..OPS {
                ...
                let p = std::pin::Pin::new(v); let r = std::pin::Pin::get_mut(p); *r = r.wrapping_add(1); black_box(*r);
            }
        })
    });
    g.bench_function(BenchmarkId::new("unsafe", &param), |b| {
        b.iter(|| {
            ...
            for t in 0..OPS {
                let v = unsafe { vals.get_unchecked_mut(t & MASK) };
                let p = unsafe { std::pin::Pin::new_unchecked(v) }; let r = unsafe { std::pin::Pin::get_unchecked_mut(p) }; *r = r.wrapping_add(1); black_box(*r);
            }
        })
    });
    g.finish();
...
criterion_group!(benches, bench_candidate_core_pin_get_unchecked_mut_1617);
