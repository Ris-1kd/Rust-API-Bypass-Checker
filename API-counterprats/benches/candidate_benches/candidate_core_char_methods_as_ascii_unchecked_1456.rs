...
fn make_inputs(n: usize, seed: u64) -> Vec<char> {
    let bytes = b"abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_-";
    let mut x = seed;
    let mut out = Vec::with_capacity(n);
    for _ in 0..n {
        x = x.wrapping_mul(6364136223846793005).wrapping_add(1);
        out.push(bytes[(x as usize) % bytes.len()] as char);
    }
    out
}

fn bench_candidate_core_char_methods_as_ascii_unchecked_1456(c: &mut Criterion) {
    let inputs = make_inputs(IN_LEN, 0x9abc_def0_1234_5678);
    let param = format!("in{}_ops{}", IN_LEN, OPS);
    let mut g = c.benchmark_group("candidate_core_char_methods_as_ascii_unchecked_1456");
...
criterion_group!(benches, bench_candidate_core_char_methods_as_ascii_unchecked_1456);
