...
fn bench_candidate_core_char_methods_from_u32_unchecked_237(c: &mut Criterion) {
    let inputs = make_valid_scalar_u32s(IN_LEN, 0x1234_5678_9abc_def0);
    let param = format!("in{}_ops{}", IN_LEN, OPS);
    let mut g = c.benchmark_group("candidate_core_char_methods_from_u32_unchecked_237");
...
criterion_group!(benches, bench_candidate_core_char_methods_from_u32_unchecked_237);
