...
fn bench_candidate_alloc_collections_btree_map_insert_before_unchecked_3461(c: &mut Criterion) {
    let keys = make_inputs();
    let param = format!("len{}_ops{}", LEN, OPS);
    let mut g = c.benchmark_group("candidate_alloc_collections_btree_map_insert_before_unchecked_3461");
...
criterion_group!(benches, bench_candidate_alloc_collections_btree_map_insert_before_unchecked_3461);
