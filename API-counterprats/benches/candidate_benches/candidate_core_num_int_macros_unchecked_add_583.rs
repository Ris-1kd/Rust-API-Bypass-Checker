...
fn bench_candidate_core_num_int_macros_unchecked_add_583(c: &mut Criterion) {
    let (xs, ys) = make_inputs(IN_LEN, 0x1234_5678_9abc_def0);
    let param = format!("in{}_ops{}", IN_LEN, OPS);
    let mut g = c.benchmark_group("candidate_core_num_int_macros_unchecked_add_583");
...
criterion_group!(benches, bench_candidate_core_num_int_macros_unchecked_add_583);
