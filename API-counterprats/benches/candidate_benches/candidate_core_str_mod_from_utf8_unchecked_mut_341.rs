...
fn bench_candidate_core_str_mod_from_utf8_unchecked_mut_341(c: &mut Criterion) {
    let mut inputs = make_inputs(IN_LEN);
    let param = format!("in{}_ops{}", IN_LEN, OPS);
    let mut g = c.benchmark_group("candidate_core_str_mod_from_utf8_unchecked_mut_341");
...
criterion_group!(benches, bench_candidate_core_str_mod_from_utf8_unchecked_mut_341);
