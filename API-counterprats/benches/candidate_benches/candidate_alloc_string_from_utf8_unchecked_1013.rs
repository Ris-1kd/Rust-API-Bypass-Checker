...
fn bench_candidate_alloc_string_from_utf8_unchecked_1013(c: &mut Criterion) {
    let mut inputs = make_inputs(IN_LEN);
    let param = format!("in{}_ops{}", IN_LEN, OPS);
    let mut g = c.benchmark_group("candidate_alloc_string_from_utf8_unchecked_1013");
...
criterion_group!(benches, bench_candidate_alloc_string_from_utf8_unchecked_1013);
