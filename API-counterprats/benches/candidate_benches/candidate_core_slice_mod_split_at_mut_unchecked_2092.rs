...
fn bench_candidate_core_slice_mod_split_at_mut_unchecked_2092(c: &mut Criterion) {
    let mids = make_mids(LEN, MID_LEN, 0x1234_5678_9abc_def0);
    let mut data: Vec<u64> = (0..LEN as u64).collect();
    let param = format!("len{}_ops{}", LEN, OPS);
    let mut g = c.benchmark_group("candidate_core_slice_mod_split_at_mut_unchecked_2092");
    g.throughput(Throughput::Elements(OPS as u64));
    g.warm_up_time(Duration::from_secs(3));
    g.sample_size(80);
    g.measurement_time(Duration::from_secs(5));
    g.bench_with_input(BenchmarkId::new("safe_split_at_mut", &param), &mids, |b, mids| {
        b.iter(|| {
            let s: &mut [u64] = black_box(&mut data[..]);
            let mids = black_box(&mids[..]);
            for t in 0..OPS {
                let mid = black_box(mids[t & MID_MASK]);
                let (l, r) = s.split_at_mut(mid);
                black_box((l.as_mut_ptr() as usize as u64) ^ (r.as_mut_ptr() as usize as u64));
            }
        })
    });
    g.bench_with_input(BenchmarkId::new("unsafe_split_at_mut_unchecked", &param), &mids, |b, mids| {
        b.iter(|| {
            let s: &mut [u64] = black_box(&mut data[..]);
            let mids = black_box(&mids[..]);
            for t in 0..OPS {
...
criterion_group!(benches, bench_candidate_core_slice_mod_split_at_mut_unchecked_2092);
