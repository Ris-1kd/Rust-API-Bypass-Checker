...
fn make_inputs(n: usize, seed: u64) -> (Vec<T>, Vec<T>) {
    ...
    for _ in 0..n {
        ...
        let a = (((x >> 32) as T) & 0xffff) + 1;
        x = x.wrapping_mul(6364136223846793005).wrapping_add(1);
        let b = (((x >> 32) as T) & 0xffff) + 1;
        xs.push(a);
        ys.push(b);
    }
    (xs, ys)
...
fn bench_candidate_core_num_uint_macros_unchecked_mul_1274(c: &mut Criterion) {
    let (xs, ys) = make_inputs(IN_LEN, 0x1234_5678_9abc_def0);
    let param = format!("in{}_ops{}", IN_LEN, OPS);
    let mut g = c.benchmark_group("candidate_core_num_uint_macros_unchecked_mul_1274");
...
criterion_group!(benches, bench_candidate_core_num_uint_macros_unchecked_mul_1274);
