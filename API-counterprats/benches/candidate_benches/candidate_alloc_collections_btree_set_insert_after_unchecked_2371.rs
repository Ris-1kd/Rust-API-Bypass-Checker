...
fn bench_candidate_alloc_collections_btree_set_insert_after_unchecked_2371(c: &mut Criterion) {
    let keys = make_inputs();
    let param = format!("len{}_ops{}", LEN, OPS);
    let mut g = c.benchmark_group("candidate_alloc_collections_btree_set_insert_after_unchecked_2371");
    ...
    g.bench_with_input(BenchmarkId::new("safe_insert_after", &param), &keys, |b, keys| {
        b.iter(|| {
            ...
            for t in 0..OPS {
                ...
                cursor.insert_after(key).unwrap();
                black_box(map.len());
            }
        })
    });
    g.bench_with_input(BenchmarkId::new("unsafe_insert_after_unchecked", &param), &keys, |b, keys| {
        b.iter(|| {
            ...
            for t in 0..OPS {
                ...
                let cursor = map.lower_bound_mut(std::ops::Bound::Unbounded);
                let mut cursor = cursor;
                unsafe { cursor.insert_after_unchecked(key) };
                black_box(map.len());
            }
        })
    });
...
criterion_group!(benches, bench_candidate_alloc_collections_btree_set_insert_after_unchecked_2371);
