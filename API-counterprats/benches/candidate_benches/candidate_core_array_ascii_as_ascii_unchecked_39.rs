...
fn bench_candidate_core_array_ascii_as_ascii_unchecked_39(c: &mut Criterion) {
    let inputs = make_inputs(IN_LEN, 0x1234_5678_9abc_def0);
    let param = format!("in{}_ops{}", IN_LEN, OPS);
    let mut g = c.benchmark_group("candidate_core_array_ascii_as_ascii_unchecked_39");
    ...
    g.bench_with_input(BenchmarkId::new("unsafe_as_ascii_unchecked", &param), &inputs, |b, inputs| {
        b.iter(|| {
            ...
            for t in 0..OPS {
                let inp = black_box(&ins[t & IN_MASK]);
                let chars = unsafe { inp.as_ascii_unchecked() };
                black_box(chars.as_ptr());
            }
        })
    });
...
criterion_group!(benches, bench_candidate_core_array_ascii_as_ascii_unchecked_39);
