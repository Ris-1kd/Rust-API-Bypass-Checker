...
fn bench_candidate_alloc_collections_btree_map_insert_before_unchecked_3641(c: &mut Criterion) {
    let keys = make_inputs();
    let param = format!("len{}_ops{}", LEN, OPS);
    let mut g = c.benchmark_group("candidate_alloc_collections_btree_map_insert_before_unchecked_3641");
    ...
    g.bench_with_input(BenchmarkId::new("safe_insert_before", &param), &keys, |b, keys| {
        b.iter(|| {
            ...
            for t in 0..OPS {
                ...
                cursor.insert_before(key, key).unwrap();
                black_box(map.len());
            }
        })
    });
    g.bench_with_input(BenchmarkId::new("unsafe_insert_before_unchecked", &param), &keys, |b, keys| {
        b.iter(|| {
            ...
            for t in 0..OPS {
                ...
                let cursor = map.lower_bound_mut(std::ops::Bound::Unbounded);
                let mut cursor = cursor;
                unsafe { cursor.insert_before_unchecked(key, key) };
                black_box(map.len());
            }
        })
    });
...
criterion_group!(benches, bench_candidate_alloc_collections_btree_map_insert_before_unchecked_3641);
