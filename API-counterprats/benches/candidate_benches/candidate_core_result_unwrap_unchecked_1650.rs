...
fn make_inputs() -> Vec<Result<u64, u64>> {
    (0..LEN as u64).map(Ok).collect()
}

fn bench_candidate_core_result_unwrap_unchecked_1650(c: &mut Criterion) {
    let inputs = make_inputs();
    let param = format!("len{}_ops{}", LEN, OPS);
    let mut g = c.benchmark_group("candidate_core_result_unwrap_unchecked_1650");
    ...
    g.bench_with_input(BenchmarkId::new("unsafe", &param), &inputs, |b, inputs| {
        b.iter(|| {
            let ins = black_box(&inputs[..]);
            for t in 0..OPS {
                let v = unsafe { ins[t & MASK].clone().unwrap_unchecked() };
                black_box(v);
            }
        })
    });
...
criterion_group!(benches, bench_candidate_core_result_unwrap_unchecked_1650);
