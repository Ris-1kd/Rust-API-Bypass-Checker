...
fn bench_candidate_core_num_int_macros_unchecked_shl_exact_1494(c: &mut Criterion) {
    let (xs, rs) = make_inputs(IN_LEN, 0xfeed_face_cafe_babe);
    let param = format!("in{}_ops{}", IN_LEN, OPS);
    let mut g = c.benchmark_group("candidate_core_num_int_macros_unchecked_shl_exact_1494");
...
criterion_group!(benches, bench_candidate_core_num_int_macros_unchecked_shl_exact_1494);
