...
fn bench_candidate_alloc_collections_btree_set_insert_after_unchecked_2457(c: &mut Criterion) {
    let keys = make_inputs();
    let param = format!("len{}_ops{}", LEN, OPS);
    let mut g = c.benchmark_group("candidate_alloc_collections_btree_set_insert_after_unchecked_2457");
...
criterion_group!(benches, bench_candidate_alloc_collections_btree_set_insert_after_unchecked_2457);
